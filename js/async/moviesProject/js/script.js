...
function displaySearchResults(results) {
    ...
    results.forEach((result) => {
        ...
        document.querySelector('#search-result-heading').innerHTML =`
            <h2>${result.length} of ${global.search.totalResults}
...
//display pagination for search
 function displayPagination() {
    ...
    document.querySelector('#next').addEventListener('click', async (e) => {
        global.search.page++;
        const {result, total_pages} = await searchAPIData();
        displaySearchResults(result, total_pages);
    });
